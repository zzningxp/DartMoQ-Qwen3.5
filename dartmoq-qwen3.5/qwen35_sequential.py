#!/usr/bin/env python3
"""Qwen3.5 MoE 顺序量化脚本，基于原 DartMoQ，只做最小改动"""
...
def construct_moe(model, moe_model_flag, layer, layer_idx, inp,
    ...
    # 检查是否是 Qwen3.5 合并权重格式
    is_qwen35_merged = is_moe_layer and hasattr(layer.mlp.experts, 'gate_up_proj')
    ...
    if moe_model_flag:
        if is_moe_layer:
            moe = reconstruct_moe_from_existing(model, layer, layer_idx, hidden_states,
                                                ...
                                                qscheme, use_hybrid_moe, global_mode, quantmode, args, is_qwen35_merged)
            layer.mlp = moe
    else:
...
@torch.no_grad()
def qwen35_dartmoq_sequential(model, tokenizer, dataloader, args, test_ppl=True):
    print('Starting ...')
