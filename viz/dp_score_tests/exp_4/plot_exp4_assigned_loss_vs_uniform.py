"""DP assigned loss by expert view.
...
Meaning:
  ...
  Each expert contributes one box over its assigned sub-expert losses.

Optional baseline:
  Add --include-uniform-baseline to also save a second boxplot:
  no sorting, no DP, no mixed precision, same target bpw as fixed bit.
  For --target-bpw 2.0, the baseline is whole-expert b2:
  each expert is split into 8 unsorted slices, and each slice uses
  sum(q_rates[b2][unsorted_slice]) * expert_activation_rate.
...
Ready-to-run command with no-sort fixed-bit baseline:
  python -m viz.dp_score_tests.exp_4.plot_exp4_assigned_loss_vs_uniform \
    --model qwen3-30b-a3b \
    --quantmode turboquant \
    --rank-mode turboquant_innerproduct \
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot DP assigned loss grouped by expert.")
    ...
    parser.add_argument("--include-uniform-baseline", action="store_true",
                        help="also save no-sort fixed-bit slice boxplot baseline")
    parser.add_argument("--comparison-out-dir", default=None,
                        help="when baseline is enabled, write both comparison boxplots here")
    parser.add_argument("--out-dir", default=os.path.join("figs", "dp_score_views"))
    args = parser.parse_args()
    args.views = ["assigned_loss_by_expert"]
    if args.include_uniform_baseline:
        args.views.append("uniform_loss_by_expert_unsorted")
        if args.comparison_out_dir is None:
            args.comparison_out_dir = os.path.join("figs", f"assigned_loss_bpw{args.target_bpw:g}_boxplot_compare")
