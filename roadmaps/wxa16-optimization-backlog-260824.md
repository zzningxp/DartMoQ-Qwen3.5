...
|---|---|---|---|---|---|
| wikitext2（145 samples） | 90.05 s | **76.32 s** | **68.96 s** | **58.01 s** | 比 FP16 快 **35.6%**，比 P6 再 -15.9% |
| c4（256 samples） | 114.7 s | **110.85 s** | **107.28 s** | **89.74 s** | 比 FP16 快 **21.8%**，比 P6 再 -16.4% |

ppl：wikitext2 7.7942 → **7.7946**；c4 11.2683 → **11.2672**（均无损失）。
...
**fast 版当前内部拆分（31.3ms）**：chunk_loop 11.4（36.4%）、cast 7.1（22.7%）、
wy_fused 5.5（17.7%）、其余 7.3。
...
**下一步候选**：
- P8-6 kernel 自动调优（BLOCK/warps/stages 搜索，参照 P5-3，预期 +5~20%）
- wy_solve 已被融合替代，不再独立优化
- P8-5 causal conv1d（需 P8-1 确认占比，当前非瓶颈）
